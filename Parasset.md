...
On-chain assets include native assets (completely decentralized and capture value on the chain) and credit assets (undertake some kind of centralized credit, such as USDT). Based on the on-chain price from the oracle, the existing native assets can be used to form all quoted assets in parallel, whether they are native or credit assets. The significance of using native assets is that the underlying assets do not take credit risk.
...
Assets which are generally decentralized and native assets on the chain, used to generate parallel assets, such as ETH, NEST, NHBTC.
...
When the price of the collateral asset drops, there is a liquidation line above the collateral rate. The liquidation line and the collateral rate conform to a certain relationship. The liquidation line is generally 10%-20% higher than the collateral rate.
...
Provides on-chain price information for collateral assets, namely NEST.
...
The insurance funds are designated to redeem day, once every 3 months, each share needs to be held for at least 3 months, and redeem according to the net value.
...
Users can deposit 1USDT to the insurance fund pool to get 1pUSD, or deposit 1pUSD for 1USDT (if there has).

### 9.Stability Fee

The stability fee is calculated according to the block. The stability fee is charged for each operation of coin minting, redemption, supplement collateral, and liquidation. See Algorithm 2 for calculation.

### 10.Insurance Net Worth

The net insurance value is calculated according to Algorithm 4.
...
For a certain debt warehouse, the total amount of mortgage assets at time t is recorded as ![image](https://latex.codecogs.com/png.latex?X_{t})，the change in collateral assets is recorded as ![image](https://latex.codecogs.com/png.latex?x_{t}),then：

![image](https://latex.codecogs.com/png.latex?\inline&space;X_{t}=X_{t-1}&plus;x_{t})

The total amount of parallel assets at time t is recorded as ![image](https://latex.codecogs.com/png.latex?Y_{t})，This parallel asset change is recorded as ![image](https://latex.codecogs.com/png.latex?y_{t}),then：
